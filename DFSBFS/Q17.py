...
# 1. Q16 문제와 매우 유사해서 자연스럽게 dfs로 풀려고 했으나 
# 결국엔 코드를 해석해보면 bfs로 풀고 있었다. 다행히 답을 맞췄다(?).
# 2. 좀 더 bfs 적으로 풀려고 큐를 이용하여 다시 풀었다. 
# 초를 나누기 위해서 next_queue를 이용하여 queue가 None이면 다시 queue에 next_queue를 
# 추가 하는 방식으로 코드를 작성하였다.